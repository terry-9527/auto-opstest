...
class ClusterInfoPage(KeyWords):
    ...
    # 添加负责人按钮
    add_person_liable_button = ('xpath', '//tbody/tr[4]//span[text()="添加负责人"]')
    # div下拉框
    ...
    # 新建集群信息
    def new_clusterinfo(self, clusterid, domain=None, comment=None, is_save=True):
    ...
    def edit_clusterinfo(self, clusterid, custemor=None, machineroom=None, domain=None, size=None, comment=None, is_save=True):
        ...
        self.wait(1)
        if custemor:
            self.div_selector(self.client_input, random.randint(0, self.list_num[0] - 1))
            self.wait(1)
        else:
            self.click_element(*self.client_input)
        if machineroom:
            self.div_selector(self.machineroom_input, random.randint(self.list_num[0], self.list_num[2] - 3))
            self.wait(1)
        else:
            self.click_element(*self.machineroom_input)
        if size:
            self.div_selector(self.sector_size_input, random.randint(self.list_num[2] - 2, self.list_num[2] - 1))
            self.wait(1)
        else:
